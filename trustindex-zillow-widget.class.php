...
class TrustindexWidget_zillow extends WP_Widget {
...
{
parent::__construct(
'trustindex_zillow_widget',
'Trustindex - Zillow reviews widget',
[
'classname' => 'trustindex-widget',
'description' => 'Embed Zillow reviews fast and easily into your WordPress site. Increase SEO, trust and sales using Zillow reviews.'
]
);
}
...
{
global $wpdb;
global $trustindex_pm_zillow;
if ($trustindex_pm_zillow->is_enabled()) {
extract($args);
echo $before_widget;
$wasError = false;
...
echo $trustindex_pm_zillow->renderWidgetFrontend($instance['ti-widget-ID']);
} else if ($trustindex_pm_zillow->is_noreg_linked()) {
echo $trustindex_pm_zillow->renderWidgetFrontend();
} else {
echo $trustindex_pm_zillow->frontEndErrorForAdmins(sprintf(__("Please fill out <strong>all the required fields</strong> in the <a href='%s'>widget settings</a> page", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_zillow->get_plugin_slug().'/settings.php')));
}
echo $after_widget;
} else {
}
...
global $wp_version;
global $trustindex_pm_zillow;
$tiWidgets = $trustindex_pm_zillow->get_trustindex_widgets();
...
<?php if ($trustindex_pm_zillow->is_trustindex_connected()): ?>
<?php if ($tiWidgets): ?>
...
<?php endforeach; ?>
<?php else: ?>
<?php echo TrustindexPlugin_zillow::get_alertbox('warning',
...
<img src="<?php echo $trustindex_pm_zillow->get_plugin_file_url('static/img/help-where-is-id.jpg'); ?>" alt="ID column here: <?php echo 'https://admin.trustindex.io/'; ?>widget" />
</div>
<?php else: ?>
<?php echo TrustindexPlugin_zillow::get_alertbox('warning',
...
. sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_zillow->get_plugin_slug().'/settings.php&tab=setup_trustindex'))
); ?>
<?php endif; ?>
</div>
<?php
